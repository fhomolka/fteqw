...
//			return;
	...
	#ifdef Q2SERVER
			case GT_QUAKE2:
				playernum = q2->sv.Q2NUM_FOR_EDICT(split->q2edict)-1;
				break;
	#endif
			default:
...
#ifdef Q2SERVER
//TEMP(fhomolka): copied over because i have no idea how to decouple this.
...
void SVQ2_BaseLines_f (void)
{
		...
		&& start < Q2MAX_EDICTS)
	{
		...
		if (base->modelindex || base->sound || base->effects)
		{
			...
			q2->sv.WriteDeltaEntity (&nullstate, base, &host_client->netchan.message, true, true);
		}
		start++;
	}
...
{	//this is the client-protocol-independant core, for q1/q2 gamecode
	...
	if (q2 && q2->sv.HasGameExport())
	{
		...
		q2->sv.ClientUserinfoChanged (split->q2edict, userinfo);	//tell the gamecode
		SV_ExtractFromUserinfo(split, true);	//let the server routines know

		q2->sv.ClientBegin(split->q2edict);
...
void SV_SetInfo_f (void)
{
	...
	else if (InfoBuf_SyncReceive(&host_client->userinfo, key, keysize, val, valsize, offset, final))
	{
		...
		if (svs.gametype == GT_QUAKE2)
		{
			...
			q2->sv.ClientUserinfoChanged (host_client->q2edict, tempbuffer);	//tell the gamecode
